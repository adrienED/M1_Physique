...
main(){
  double somme;
    ...
    */
    ...
    for(j=1;j<M;j++){
      t=j*Dt;
      for(n=1;n<N;n++){//update for c[n] and a[n]
	c_temp = c[n] + Dt*a[n];
	a_temp = a[n] - Dt*(v2*eigenvalue(n)*c[n]+g2*b[n]);
	c[n] = c_temp; a[n] = a_temp;
	
      
      }
    
    
//calculer la nouvelle valeur de phi(x) à l'instant actuel
      for(i=1;i<(N+2);i++){
	x = i*(L/(N+1));
	somme = 0.0;
	for(n=1;n<N;n++){
	...
	f[i] = sin(somme)*eigenfunction(n,x);
	phi[i] = somme;

      }
 //mise a jour des coefficients b[n]
      for(n=1;n<N;n++){
	b[n] = integrate(f,s,0.0,L,N);
      }
      if((j%100)==0){
